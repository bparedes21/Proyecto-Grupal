...
# IMPORTANTE: " Parámetro a pasar a estas funciones"
#           Diccionario que contenga cada df como valor y le asigne como key el nombre
...
# drop_colums: en caso de haber alguna columna que no deba ingresarse en la base de datos
# esta funcion la elimina.
def drop_columns(dicc_df):
    for key in dicc_df:
        for columna in dicc_df[key].columns:
            if columna=='Unnamed: 0':
                dicc_df[key].drop(columns=['Unnamed: 0'], inplace=True)
